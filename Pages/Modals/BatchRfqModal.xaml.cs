    ...
    public partial class BatchRfqModal : ContentView
    {
        ...
        private void OnBatchRfqUnitPriceTextChanged(object? sender, TextChangedEventArgs e)
        {
            if (sender is Entry entry && entry.BindingContext is RfqItem item)
            {
                ...
                {
                    // The item's PropertyChanged subscription already drives the recalc; the
                    // explicit calls in these handlers doubled every pass.
        ...
        private void OnBatchRfqTotalDiscountTextChanged(object? sender, TextChangedEventArgs e)
        {
            ...
            var resolved = DiscountInput.Resolve(e.NewTextValue, ViewModel.CalculatedBatchRfqBaseTotal);
            if (resolved.HasValue && ViewModel.BatchRfqDiscount != resolved.Value)
            {
                ViewModel.BatchRfqDiscount = resolved.Value;
            }
        }

        // The BatchRfqVatType / BatchRfqCurrency partial hooks on the page model already run the
